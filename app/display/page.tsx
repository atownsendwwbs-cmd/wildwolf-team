...
export default async function DisplayPage() {
  ...
  return (
    <div className="min-h-screen bg-sky-900 text-white p-10">
      <AutoRefresh seconds={30} />
      <div className="h-1.5 bg-gradient-to-r from-orange-600 via-orange-300 to-orange-600 -mt-10 -mx-10 mb-8" />

      <header className="flex items-center justify-between mb-10">
        <h1 className="text-3xl font-display font-bold uppercase tracking-wide">
          Wild <span className="text-orange-200">Wolf</span> Warehouse
        </h1>
      ...
      <div className="grid grid-cols-3 gap-8">
        <section className="col-span-2 space-y-6">
          <div>
            <h2 className="text-lg font-semibold uppercase tracking-wide text-orange-200 mb-3">
              Announcements
            </h2>
            {announcements.length === 0 ? (
              <p className="text-2xl text-white/50">Nothing posted yet today.</p>
            ) : (
              <ul className="space-y-4">
                {announcements.map((a) => (
                  <li key={a.id} className="rounded-xl border border-white/10 bg-white/5 p-5">
        ...
        <section>
          <h2 className="text-lg font-semibold uppercase tracking-wide text-red-200 mb-3">
            Critical Inventory
          </h2>
          {criticalAlerts.length === 0 ? (
            <p className="text-xl text-white/50">Nothing critical. 🎉</p>
          ) : (
            <ul className="space-y-3">
              {criticalAlerts.map((alert) => (
                <li key={alert.id} className="rounded-xl border border-red-400/40 bg-red-900/40 p-4">
                  <p className="text-xl font-semibold">{alert.itemName}</p>
                  {alert.category === "RAW_MATERIAL" && alert.quantity && (
                    <p className="text-base text-red-200 mt-1">Left: {alert.quantity}</p>
                  )}
                </li>
              ))}
            </ul>
